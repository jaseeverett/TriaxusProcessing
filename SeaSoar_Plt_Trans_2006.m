...
if ii == 1
    [XI,YI,ZI] = find_downcast(grnddist,Depth,s.temperature);
    cont_res = 13:0.25:24;
    lcont_res = 13:24;
    ...
    [XI,YI,ZI] = find_downcast(grnddist,Depth,s.sigma);
    cont_res = 1024:.1:1028;
    lcont_res = 1024:.5:1028;
    [c1,h1] = contour(XI,YI,ZI,lcont_res,'color',cl);
    set(h1(:),'Linewidth',1);
    clabel(c1,h1,'labelspacing',200,'Fontweight','b','color',cl);
    for ci=1:length(cont_res)
        if isempty(find(cont_res(ci) == lcont_res))
            [c1,h1] = contour(XI,YI,ZI,[cont_res(ci) cont_res(ci)],'color',cl);
            set(h1(:),'Linewidth',0.35);
        end
    end
end
...
% Cruise Track
%Depth(1:50:end) = NaN;
h2 = scatter(grnddist(1:8:end), Depth(1:8:end),'.','MarkerEdgeColor',[0.5 0.5 0.5],'SizeData',4);
